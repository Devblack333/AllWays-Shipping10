...
/**
 * Build candidate filenames for a key and language.
 */
...
/* Candidate folders to try (includes your "image/" folder) */
...
/* Probe a URL quickly — resolves true if image loads */
...
/* Resolve a single img element that has data-img-key */
async function resolveImageElement(imgEl) {
  ...
  for (const prefix of IMAGE_PREFIXES) {
    for (const name of filenames) {
      ...
      // probe synchronously in loop (await) so we pick first valid
...
/* Resolve all data-img-key images (fire-and-forget) */
